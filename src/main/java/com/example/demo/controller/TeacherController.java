...
public class TeacherController {
  ...
  @Operation(summary = "Fetching a Teacher Details", description = "This accept courseId as a path variable and teacher as RequestBody and returns TeacherDTO as a object", parameters = {
          ...
          @Parameter(name = "id", description = "Accept a CourseID as a path variable.", required = true)}) @ApiResponses(value = {
                  ...
                  @ApiResponse(responseCode = "302", description = "This throw's when teacher already exist.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = com.example.demo.exception.ApiResponse.class)))}) @PreAuthorize("hasAuthority('ADMIN')") @PostMapping("/course/{id}")
  public ResponseEntity<?> createTeacher(@RequestBody
  Teacher teacher, @PathVariable(name = "id")
  String id) {
